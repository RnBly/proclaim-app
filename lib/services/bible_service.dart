  ...
  */
  ...
  String formatSelectedVersesEsv(List<SelectedVerseEsv> verses) {
    ...
    void writeRange() {
      ...
      if (currentRange.length == 1) {
        buffer.writeln('[${first.bookEng} ${first.chapter}:${first.verseNumber}]');  // bookEng 사용
        buffer.writeln('${first.verseNumber}. ${first.text}');
      } else {
        buffer.writeln('[${first.bookEng} ${first.chapter}:${first.verseNumber}-${last.verseNumber}]');  // bookEng 사용
        for (var verse in currentRange) {
          buffer.writeln('${verse.verseNumber}. ${verse.text}');
        }
      }
